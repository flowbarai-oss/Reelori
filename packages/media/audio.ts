...
export async function saveAudio(bytes:Buffer,dir:string){
 ...
 if(bytes.length>10*1024*1024||bytes.length<12||(!wav&&!mp3))throw new DomainError('请使用 10 MB 内的 WAV 或 MP3 音频');
 ...
 try{
  ...
  try{await ffmpeg(['-protocol_whitelist','file,pipe','-i',input,'-map','0:a:0','-vn','-t','31','-ar','48000','-ac','2','-c:a','pcm_s16le','-f','s16le','-y',raw],30000);}catch{throw new DomainError('无法解码音频；原文件未加入项目');}
  const pcm=await readFile(raw),durationMs=Math.ceil(pcm.length/192);
  if(durationMs<100||durationMs>30000||pcm.length%4)throw new DomainError('音频须为 0.1–30 秒；不会静默截断较长音频');
