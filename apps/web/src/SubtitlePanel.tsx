...
export function SubtitlePanel({project,lang,request,onChange,onDirty}:{project:Project;lang:string;request:(path:string,body?:unknown)=>Promise<any>;onChange:(p:Project)=>void;onDirty:(dirty:boolean)=>void}){
 ...
 return <details className="audio-panel"><summary>{t('字幕文本与时间','Subtitle text and timing')}</summary>
  ...
  {cues.map((c,i)=><div className="audio-track" key={i}><div className="audio-form">
   <label>{t('开始秒数','Start seconds')}<input type="number" min="0" max="30" step="0.001" value={c.startMs/1000} disabled={busy} onChange={e=>edit(i,{startMs:Math.round(Number(e.target.value)*1000)})}/></label>
   <label>{t('结束秒数','End seconds')}<input type="number" min="0" max="30" step="0.001" value={c.endMs/1000} disabled={busy} onChange={e=>edit(i,{endMs:Math.round(Number(e.target.value)*1000)})}/></label>
  ...
  <button className="secondary" disabled={busy||cues.length>=60} onClick={()=>{const end=cues.at(-1)?.endMs??0;setCues([...cues,{startMs:end,endMs:Math.min(30000,end+1000),text:''}]);markDirty();}}>{t('添加字幕','Add cue')}</button>
