...
H3 uses POST `/video_generation` with `model: MiniMax-H3`, text `content`, `duration: 5`, `resolution: 768P`, explicit `ratio`. The China route can also include one uploaded `reference_image` only after the user selects it and confirms rights in the quote; the local API enforces the same confirmation. Asset hash and H3 image dimensions are checked before quoting and before submitting. Query GET `/query/video_generation/{id}` and require response task identity to match. Download the returned public HTTPS URL without the API key. Local media validation still applies. Known tasks automatically resume query-only recovery; unsent reservations and restored project copies never auto-submit.
